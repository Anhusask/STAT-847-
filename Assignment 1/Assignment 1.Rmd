...
```{r}
lm.fit3 = lm(mpg~.+I(cylinders^2) +  I(displacement^2) + I(horsepower^2) + I(weight^2) + I(acceleration^2) + I(year^2), Auto[,-9])
summary(lm.fit3)
```
```{r}
lm.fit4 = lm(mpg~.+log(cylinders) +  log(displacement) + log(horsepower) + I(log(weight)) + I(log(acceleration)) + I(log(year)), Auto[,-9])
summary(lm.fit4)
```
```{r}
